...
void PatchMatchStereo::RandomInitialization() const
{
	...
	for (int k = 0; k < 2; k++) {
		...
		for (sint32 y = 0; y < height; y++) {
			for (sint32 x = 0; x < width; x++) {
				...
				// 随机视差值
				float32 disp = sign * rand_d(gen);//左视图 正的视差，负的视差
				...
				if (!option.is_fource_fpw) {
					...
					norm.z = z;
					norm.normalize();
					norm.flag_left = false;
				}
				else {
					norm.x = 0.0f; norm.y = 0.0f; norm.z = 1.0f; norm.flag_left = false;
				}
	...
	/*for (int i = 0; i < 1242; i++)
		std::cout << disp_right_[114 * width + i] << std::endl;*/

		for (int y = 130; y < 375; y++) {
			for (int x = 0; x < 1242; x++) {
				//添加左视图
				float disp = option.leftlidarimg->at<cv::Vec3f>(y, x)[0];
				if (disp > 0)
				{
					disp_left_[y * width + x] = disp;
					//is_lidar_[y * width + x] = 1;//所有的插入点
					//PVector3f norm(option.leftnormalimg->at<cv::Vec3f>(y, x)[0], option.leftnormalimg->at<cv::Vec3f>(y, x)[1], option.leftnormalimg->at<cv::Vec3f>(y, x)[2]);//雷达点云的法向量
					
					plane_left_[x + y * width] = DisparityPlane(x, y, plane_left_[x + y * width].to_normal(), disp);
				}
				//添加右视图  右视图的视差都是负数
				disp = option.rightlidarimg->at<cv::Vec3f>(y, x)[0];
				if (disp < 0)
				{
					disp_right_[y * width + x] = disp;
					//is_lidar_[y * width + x] = 1;//所有的插入点
					//PVector3f norm(option.rightnormalimg->at<cv::Vec3f>(y, x)[0], option.rightnormalimg->at<cv::Vec3f>(y, x)[1], option.rightnormalimg->at<cv::Vec3f>(y, x)[2]);//雷达点云的法向量
					
					plane_right_[x + y * width] = DisparityPlane(x, y, plane_right_[x + y * width].to_normal(), disp);
				}
	...
	////显示视差图
	cv::Mat disp_mat = cv::Mat(height, width, CV_8UC1, cv::Scalar(0));
	for (sint32 y = 0; y < height; y++) {
		for (sint32 x = 0; x < width; x++) {
			disp_mat.at<uchar>(y, x) = disp_left_[y * width + x];
		}
	}
	cv::equalizeHist(disp_mat,disp_mat);
	cv::imshow("first", disp_mat);
	//cv::waitKey(1000);
}
...
void PatchMatchStereo::Propagation() const
{
	...
	option_right.direction = true;
	option_left.direction = false;
	//左右option的区别只是最大视差变成负数
	// 左右视图传播实例
	
	//颠倒左视图和右视图的参数位置
	PMSPropagation propa_left(width, height, img_left_, img_right_, grad_left_, grad_right_, plane_left_, plane_right_, 
		option_left,cost_left_,cost_right_, disp_left_,gray_left_,gray_right_);//计算初始代价
	PMSPropagation propa_right(width, height, img_right_, img_left_, grad_right_, grad_left_, plane_right_,plane_left_, 
		option_right, cost_right_, cost_left_, disp_right_,  gray_left_, gray_right_);
