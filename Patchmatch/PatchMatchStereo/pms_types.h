...
struct PMSOption {
	...
	uint16  number;//裁掉超过图像的部分点云
	
	bool direction;//0代表左，1代表右视图。
	bool* flag_right,*flag_left;
	bool* is_rightlidar,*is_leftlidar;

	cv::Mat *rightlidarimg,*leftlidarimg;
	//cv::Mat *rightnormalimg,*leftnormalimg;

	PMSOption() : patch_size(35), min_disparity(0), max_disparity(64), gamma(10.0f), alpha(0.9f), tau_col(10.0f),
	              tau_grad(2.0f), num_iters(3),
	              is_check_lr(false),
	              lrcheck_thres(0),
	              is_fill_holes(false), is_fource_fpw(false), is_integer_disp(false), number(0),flag_right(nullptr),flag_left(nullptr),
				  is_rightlidar(nullptr),is_leftlidar(nullptr),direction(false) { }
};
...
struct PVector3f {
	...
	PVector3f() = default;
	PVector3f(const float32& _x, const float32& _y, const float32& _z,bool Flag) {
		x = _x; y = _y; z = _z; flag_left = Flag;
	}
	PVector3f(const uint8& _x, const uint8& _y, const uint8& _z,bool Flag) {
		x = float32(_x); y = float32(_y); z = float32(_z); flag_left = Flag;
	}
...
struct DisparityPlane {
	...
	DisparityPlane(const float32& x, const float32& y, const float32& z,bool Flag) {
		p.x = x; p.y = y; p.z = z; p.flag_left = Flag;
	}
	DisparityPlane(const sint32& x, const sint32& y, const PVector3f& n, const float32& d,bool Flag) {
		p.x = -n.x / n.z;
		p.y = -n.y / n.z;
